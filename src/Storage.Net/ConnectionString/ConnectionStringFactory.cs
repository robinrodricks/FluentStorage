...
namespace Storage.Net.ConnectionString
{
   static class ConnectionStringFactory
   {
      ...
      public static IBlobStorage CreateBlobStorage(string connectionString)
      {
         ...
         if (instance == null) throw new ArgumentException("could not create any implementation based on the passed connection string, did you register required external module?", nameof(connectionString));

...
public class ConnectionString {
	...
	/// <summary>
	/// Creates a new instance of <see cref="FluentStorage.ConnectionStrings.ConnectionString"/>
	/// </summary>
	...
	private void Parse(string connectionString) {
		...
		if (connectionString.StartsWith(ConnectionStringParam.Native + "=")) {
			_nativeConnectionString = connectionString.Substring(ConnectionStringParam.Native.Length + 1);
			_parts[ConnectionStringParam.Native] = _nativeConnectionString;
		}
		else {
	...
	public override string ToString() {
		...
		if (IsNative) {
			sb.Append(ConnectionStringParam.Native);

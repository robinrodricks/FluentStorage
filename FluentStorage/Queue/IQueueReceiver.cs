...
public interface IQueueReceiver : IDisposable {
	...
	/// </summary>
	Task DeadLetterMessage(QueueMessage message, string reason, string errorDescription, CancellationToken cancellationToken = default);
	...
	Task<List<QueueMessage>> PeekMessages(int maxMessages, CancellationToken cancellationToken = default);
	...
	/// </summary>
	Task StartMessagePump(Func<List<QueueMessage>, CancellationToken, Task> onMessageAsync, int maxBatchSize = 1, CancellationToken cancellationToken = default);
	...
	/// <param name="cancellationToken"></param>
	/// <returns></returns>
	Task KeepAlive(QueueMessage message, TimeSpan? timeToLive = null, CancellationToken cancellationToken = default);
